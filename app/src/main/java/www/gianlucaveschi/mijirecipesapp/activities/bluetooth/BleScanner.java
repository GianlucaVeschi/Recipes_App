...
public class BleScanner {
    ...
    public BleScanner(Context context) {
        ...
        if (bluetooth_adapter == null || !bluetooth_adapter.isEnabled()) {
            Log.d(Constants.MIJI_TAG, "Bluetooth is NOT switched on");
            Intent enableBtIntent = new Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE);
            enableBtIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(enableBtIntent);
        }
        Log.d(Constants.MIJI_TAG, "Bluetooth is switched on");
    }
    ...
    public void startScanning(final ScanResultsConsumer scan_results_consumer, long stop_after_ms) {
        ...
        if (scanning) {
            Log.d(Constants.MIJI_TAG, "Already scanning so ignoring startScanning request");
            return;
        }
        //this object will start the Bluetooth scanning process
        if (scanner == null) {
            scanner = bluetooth_adapter.getBluetoothLeScanner();
            Log.d(Constants.MIJI_TAG, "Created BluetoothScanner object");
        }
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (scanning) {
                    Log.d(Constants.MIJI_TAG, "Stopping scanning");
        ...
        Log.d(Constants.MIJI_TAG, "Scanning");
